        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20 gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="hidden md:flex flex-wrap gap-1 sm:gap-2 overflow-x-auto">
                <button id="tour-nav-jobs" onClick={() => setView('listings')} className={`px-3 py-1.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all whitespace-nowrap ${view === 'listings' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>My Jobs</button>
                <button onClick={() => setView('applicants')} className={`px-3 py-1.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all whitespace-nowrap ${view === 'applicants' || view === 'applicant_detail' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>Applicants ({incomingApps.length})</button>
                <button id="tour-nav-post" onClick={() => setView('post')} className={`px-3 py-1.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all whitespace-nowrap ${view === 'post' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>Post</button>
                <button id="tour-nav-talent" onClick={() => setView('candidates')} className={`px-3 py-1.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all whitespace-nowrap ${view === 'candidates' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>Talent</button>
              </div>
            </div>
             ...
             {activeJobs.length === 0 ? (
               ...
               <div className="grid gap-6">
                 {activeJobs.map(job => {
                   ...
                   return (
                     <div key={job.id} className={`relative bg-white dark:bg-slate-800 rounded-3xl shadow-sm border transition-all overflow-hidden ${isMatching ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-100 dark:border-slate-700 hover:shadow-lg'}`}>
                       <div className="p-6 sm:p-8">
                         <div className="absolute right-6 top-6 text-right">
                           <span className="text-2xl sm:text-3xl font-black text-blue-600 leading-none">{appCount}</span>
                           <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Applicants</div>
                         </div>
                         <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 pr-20 sm:pr-28">
                           <div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 pb-safe z-50">
        <div className="flex justify-between items-center h-16 px-3">
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-2xl transition-all ${view === 'listings' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}
          >
            <Briefcase size={20} strokeWidth={view === 'listings' ? 2.5 : 2} />
            <span className="text-[10px] font-bold uppercase tracking-wide whitespace-nowrap">Jobs</span>
          </button>
          <button
            onClick={() => setView('applicants')}
            className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-2xl transition-all ${view === 'applicants' || view === 'applicant_detail' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}
          >
            <Users size={20} strokeWidth={view === 'applicants' || view === 'applicant_detail' ? 2.5 : 2} />
            <span className="text-[10px] font-bold uppercase tracking-wide whitespace-nowrap">Apps {incomingApps.length > 0 ? `(${incomingApps.length})` : ''}</span>
          </button>
          <button
            onClick={() => setView('post')}
            className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-2xl transition-all ${view === 'post' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}
          >
            <PlusCircle size={20} strokeWidth={view === 'post' ? 2.5 : 2} />
            <span className="text-[10px] font-bold uppercase tracking-wide whitespace-nowrap">Post</span>
          </button>
          <button
            onClick={() => setView('candidates')}
            className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-2xl transition-all ${view === 'candidates' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}
          >
            <Search size={20} strokeWidth={view === 'candidates' ? 2.5 : 2} />
            <span className="text-[10px] font-bold uppercase tracking-wide whitespace-nowrap">Talent</span>
          </button>
        </div>
      </div>
    </div>
